...
def get_smina_path():
    """sminaの実行可能ファイルのパスを動的に検出"""
    # 1. 環境変数から取得を試行
    if 'SMINA_PATH' in os.environ:
        smina_path = os.environ['SMINA_PATH']
        if os.path.isfile(smina_path) and os.access(smina_path, os.X_OK):
            return smina_path

    # 2. PATHからsminaコマンドを検索
    smina_path = shutil.which("smina")
    if smina_path:
        return smina_path

    # 3. ローカルバイナリを検索（フォールバック）
    local_paths = [
        os.path.join(os.path.dirname(__file__), "smina.osx.12"),
        os.path.join(os.path.dirname(__file__), "smina.static"),
        os.path.join(os.path.dirname(__file__), "smina")
    ]
    ...
    raise FileNotFoundError(
        "smina executable not found. Please ensure smina is installed via conda:\n"
        "  conda install -c conda-forge smina\n"
        "Or set SMINA_PATH environment variable to the smina executable path."
    )
